...
export default class Heap {
  ...
  allocate: (heapobj: HeapObject) => HeapObject;
  ...
  insertBefore(index: number, heapobj: HeapObject): HeapObject {
    let gap;
  ...
  objAtLoc(loc: number): HeapObject | null {
  ...
  removeObject(heapObj: HeapObject): void {
    this.removeObjectAtIndex(this.objects.indexOf(heapObj));
  }
  removeObjectAtIndex(i: number): void {
    this.objects.splice(i, 1);
    return;
  }

  firstfit(heapobj: HeapObject): HeapObject {
    for (let i = 0; i <= this.objects.length; i++) {
      try {
  ...
  bestfit(heapobj: HeapObject): HeapObject {
    if (this.objects.length == 0) {
    ...
    let lst = this.objects[this.objects.length - 1];
  ...
  worstfit(heapobj: HeapObject): HeapObject {
    if (this.objects.length == 0) {
      this.insertBefore(0, heapobj);
      return heapobj;
    }
